...
| **P2 — Observation canvas** (wk 8–12, ∥) | Radar / satellite / lightning | LibreWXR confirmed live (G3) — still build the failover chain (RainViewer → Iowa IEM) in this phase, not later | ~25–30 h | Radar loop animates; **failover exercised by blocking LibreWXR in dev** |
| **P3 — Trust moat** (wk 13–20) | *Who Was Right?* + *Personal Nowcast* | **`aviationweather.gov` has no CORS (§0.3)** — the METAR observation side needs the Worker proxy or Tauri native. Resolve at P3 start, not P3 end | ~40–50 h | After 30 days of logging: leaderboard shows a meaningful per-model ranking for home |
