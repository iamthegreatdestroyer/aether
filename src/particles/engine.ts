/**
 * The wind particle engine — the spike's G0.4-winning baseline, promoted into the app.
 *
 * Provenance matters here: this exact code (shaders verbatim) measured 60 fps at 2,002,225
 * particles on integrated AMD graphics in Chrome AND within +/-1% of that inside Tauri's
 * WebView2 at an identical canvas — while both evaluated libraries fell short (one capped at
 * 30 fps by scheduling, one 2.8x less efficient per primitive; spike/README.md has the full
 * three-way table). Zero dependencies, MIT lineage (cambecc/earth -> windgl algorithm).
 *
 * The technique, unchanged since Beccario: positions live in a texture (2 bytes/axis);
 * an update pass advects them through the wind field in a ping-pong FBO; a draw pass renders
 * one GL_POINT per particle into a persistent screen texture that fades each frame, which is
 * what produces trails. All state is on the GPU — the CPU does 2 draw calls and ~8 uniform
 * writes per frame.
 *
 * Two hard-won details preserved from the spike, do not "simplify" either:
 *   - the position codec uses 255, not 256 (256 compounds a +0.392%/frame drift that renders
 *     a plausible-looking but wrong wind field);
 *   - crypto.getRandomValues() is chunked (it throws above 65,536 bytes per call, and a
 *     million particles needs a 4 MB seed).
 */
...
/** The decoded wind texture plus the scaling needed to invert it. */
...
export class WindEngine {
  ...
  /**
   * Zero the trail buffers. Called on map movestart: the framebuffer-trails approach smears
   * under camera motion (the known trade against geometric trails); a clear at gesture start
   * reads as intentional rather than broken.
   */
  clearTrails(): void {
    if (this.width > 0) this.resize(this.width, this.height);
  }
